...
var PROBLEM_SPEED = 5000; //интервал между созданием преград
...
function startGame() {
    if(!gameIsRunning) {
        ...
        setTimeout(createWall, PROBLEM_SPEED);        // запускаем ф-цию генерирующую препятствия
    }
...
function headTeleport (coord_y, coord_x) {
    ...
    if(direction == 'x-'){
        ...
        unit = document.getElementsByClassName('cell-' + (FIELD_SIZE_X - 1) + '-' + (coord_x))[0];
    }
    else if(direction == 'y-') {
        unit = document.getElementsByClassName('cell-' + (0) + '-' + (cpprd_x))[0];
    }
    return unit;
}
...
/*
*
* проверка, не врезалась змейка в стену
*
 */
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        scoreBlock.innerText = 'Счет: ' + score;
    }
...
function createWall() {
    ...
    while (!wallCreated) { //пока стена не создана
        // рандом
